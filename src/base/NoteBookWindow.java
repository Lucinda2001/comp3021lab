...
public class NoteBookWindow extends Application {
	...
	private HBox addHBox() {
		...
		hbox.getChildren().addAll(buttonLoad, buttonSave,label, textfield, buttonSearch, buttonClear);
	...
	private void updateListView() {
		...
		for(Folder f:this.noteBook.getFolders()){
			if(f.getName().equals(currentFolder)){
				for(Note n: f.getNotes()){
					if(n instanceof TextNote && currentSearch == "")
						list.add(n.getTitle());
					else if(n instanceof TextNote && currentSearch != "" && f.searchNotes(currentSearch).contains(n)){
						list.add(n.getTitle());
					}
				}
			}
